...
3. Recién ahí, `reports_folder` está aislado del correo real — para que `parsedmarc` **no mueva ni borre absolutamente nada** (que el correo quede tal cual, sin crear ninguna subcarpeta), usar `test = True` (ver `config/parsedmarc.ini.example`). Es seguro porque `ingest_aggregate_report()`/`ingest_forensic_report()` (`services/reports_service.py`/`services/forensic_reports_service.py`) son **idempotentes**: si el mismo correo se vuelve a leer en el próximo ciclo (con `watch=True`, o simplemente porque nunca se movió), no se duplica en la base — se detecta por `report_id`/`message_id` y se ignora. Sin ese cambio, `test=True` habría sido riesgoso (reportes duplicados en cada ciclo).

Alternativas dentro de esa misma etiqueta aislada, si se prefiere no acumular correos para siempre: `delete = True` (borra tras procesar, tampoco crea subcarpetas) o dejar el archivado default (crea `Archive/Aggregate`, etc., pero aislado — nunca toca correo real).
